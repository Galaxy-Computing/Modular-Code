...
-- Modified by eli310 for the Modular OS
--
...
function shell.resolveProgram(command)
    ...
    for sPath in string.gmatch(sPath, "[^:]+") do
        ...
        if fs.exists(sPath) and not fs.isDir(sPath) then
            ...
            local sPathLua = pathWithExtension(sPath, "lua")
            if fs.exists(sPathLua) and not fs.isDir(sPathLua) then
                if sPathLua == "rom/programs/shell.lua" then
                    return "modular/modules/shell/main.lua"
                end
                return sPathLua
            end
        end
    ...
    -- Or look in modules
    for _,sModule in ipairs(module.list()) do
        if sModule == command then
            if module.isExecutable(sModule) then
                return "/modular/modules/"..sModule.."/main.lua"
            end
        end
    end
...
function shell.programs(include_hidden)
    ...
    -- Add executable modules (Modular OS)
    for i,sModule in ipairs(module.list()) do
        if module.isExecutable(sModule) then
            tItems[sModule] = true
        end
    end
...
if multishell then
    ...
    function shell.openTab(...)
        ...
        if sCommand then
            ...
            if sPath == "modular/modules/shell/main.lua" then
                return multishell.launch(createShellEnv("rom/programs"), sPath, table.unpack(tWords, 2))
            elseif sPath ~= nil then
                return multishell.launch(createShellEnv("rom/programs"), "modular/modules/shell/main.lua", sCommand, table.unpack(tWords, 2))
            else
                printError("No such program")
            end
        end
...
if #tArgs > 0 then
    ...
    local function show_prompt()
        ...
        if module.currentUser == "root" then
            term.setTextColour(colours.red)
        else
            term.setTextColour(colours.lime)
        end
        write(module.currentUser)
    ...
    print("shell "..module.version("shell"))
    term.setTextColour(textColour)
